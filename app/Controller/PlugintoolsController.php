...
class PlugintoolsController extends AppController {
    ...
    public function run($id){ 
       $funcs = $this->Plugintool->Functionality->query("SELECT `function_name`,`name` from `functions` f inner join plugins p  where f.plugin_id = p.id ");   
        
       $plugin = $this->Plugintool->read(null,$id);
        $selectedPlugins = $plugin;
          
        
        foreach($selectedPlugins['Functionality'] as $plugins){
            $selectedFunctions[] = $plugins['function_name'];            
        }


           
        $this->set('selectedFunctions',$selectedFunctions);
     //  $this->set('f_names', $f_names);
